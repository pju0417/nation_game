...
function startGame() {
  ...
  var firstHuman = -1;
  for (var i=0;i<G.players.length;i++) { if (!G.players[i].isAI){ firstHuman=i; break; } }
...
function advanceHuman() {
  ...
  if (next>=G.players.length) {
    G.players.forEach(function(p){ if(p.isAI) p.action=aiDecide(p); });
    processTurn();
    G.phase = 'events';
  } else {
    G.curPlayer = next;
...
function nextTurn() {
  ...
  G.turn++;
  var fh = -1;
  for (var i=0;i<G.players.length;i++){ if(!G.players[i].isAI){ fh=i; break; } }
  G.curPlayer = fh>=0?fh:0;
  G.phase     = fh>=0?'cover':'events';
...
function tabDiplo()    { G.tab='diplo';    render(); }
...
function restartGame() { initSetup(G.pCount); G.phase='setup'; render(); }
function updCount(n)   { G.pCount=n; initSetup(n); render(); }
function updClimate(i,v){ G.pConfigs[i].climate=v; render(); }
function updName(i,v)  { G.pConfigs[i].name=v; }
function setAI(i,v)    { G.pConfigs[i].isAI=v; if(v) G.pConfigs[i].name='AI '+(i+1); render(); }
...
function sendTrade() {
  ...
  var giveResEl  = document.getElementById('tradeGiveRes');
  var giveAmtEl  = document.getElementById('tradeGiveAmt');
  var recvResEl  = document.getElementById('tradeRecvRes');
  var recvAmtEl  = document.getElementById('tradeRecvAmt');

  if (!targetEl||!giveResEl||!giveAmtEl||!recvResEl||!recvAmtEl) return;

  var toId    = parseInt(targetEl.value);
  var giveRes = giveResEl.value;
  var giveAmt = parseInt(giveAmtEl.value)||1;
  var recvRes = recvResEl.value;
  var recvAmt = parseInt(recvAmtEl.value)||1;
  var p       = G.players[G.curPlayer];

  if (giveRes===recvRes) { alert('같은 자원끼리는 교역할 수 없습니다.'); return; }
  if ((p.resources[giveRes]||0) < giveAmt) { alert('자원이 부족합니다.'); return; }

  makeTrade(p.id, toId, giveRes, giveAmt, recvRes, recvAmt);
  alert('교역 제안을 보냈습니다! 상대방이 수락하면 턴 종료 시 처리됩니다.');
  render();
}
...
function render() {
  ...
  /* 세계지도 그리기
     - playerTurn: 현재 플레이어 하이라이트 + 펄스 애니메이션
     - events / summary / results: 정적 1회 표시
     - 온라인 화면도 동일 */
  if (typeof scheduleMapDraw === 'function') {
    var needsMap = ['playerTurn','events','summary','results','ol-game','ol-events','ol-summary','ol-results'];
    if (needsMap.indexOf(G.phase) !== -1) {
      var players = G.players || [];
      var curId   = (G.phase === 'playerTurn') ? G.curPlayer
                  : (G.phase === 'ol-game')    ? (typeof OL !== 'undefined' ? OL.myPlayerId : null)
                  : null;
      scheduleMapDraw(players, curId);
    }
  }
}
  ...
  function resize() {
    ...
    for (var i=0;i<160;i++) {
      stars.push({ x:Math.random()*canvas.width, y:Math.random()*canvas.height,
                   r:Math.random()*1.4+0.4, a:Math.random(), sp:Math.random()*0.008+0.003 });
    }
  }
