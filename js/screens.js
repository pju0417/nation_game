/* =========================================================
   ...
   onclick 작성 원칙:
   - 인자 없는 함수: onclick="goMenu()"
   - 문자열 인자:   onclick="selBuild(\'' + id + '\')"
     → JS 문자열 안에서 \' 는 ' 로 출력됨
     → HTML 결과: onclick="selBuild('farm')"
   - 숫자/boolean:  onclick="setAI(' + i + ',this.checked)"
========================================================= */
...
function renderMenu() {
  ...
  html += '세계 여러 <strong style="color:var(--gold);">기후 지역</strong>의 나라를 맡아 경영하는 <strong style="color:var(--gold);">문명 시뮬레이션</strong>입니다.<br>';
  html += '건물을 짓고, 기술을 연구하고, 정책을 펼쳐 나라를 발전시키세요!<br>';
  ...
  html += '<div style="margin-top:28px;font-size:0.75em;color:var(--text2);letter-spacing:0.5px;">';
  html += '🌴 열대 · 🏜️ 건조 · 🌿 온대 · 🌲 냉대 · 🌾 계절풍 · ⛰️ 고산';
  html += '</div></div></div>';
  return html;
}
...
function renderPlayerTurn() {
  ...
  var bc = buildCount(p);
  html += '<div class="panel">';
  html += '<div class="section-title">🏗️ 건물 (' + p.buildings.length + '개)</div>';
  if (p.buildings.length === 0) {
    html += '<div style="color:var(--text2);font-size:0.82em;">아직 없습니다</div>';
  } else {
    var shown = {};
  ...
  html += '<div class="panel">';
  html += '<div class="section-title">🔬 기술 (' + p.techs.length + '개)</div>';
  if (p.techs.length === 0) {
    html += '<div style="color:var(--text2);font-size:0.82em;">아직 없습니다</div>';
  } else {
  ...
  /* tabBuild/tabResearch/tabPolicy/tabScores 는 main.js에 정의 */
  ...
  G.players.forEach(function (op) {
    if (op.id !== p.id) html += miniCard(op);
  });
  html += '</div>';
...
/*
  ...
  - \' 는 JS 문자열 안에서 ' 로 출력됨
  - 결과: onclick="selBuild('farm')"  ← 올바른 HTML
*/
...
function renderPolicyTab() {
  ...
  POLICIES.forEach(function (pol) {
    ...
    /* ★ pol.id 값이 실제로 출력되도록 문자열 연결 */
    html += '<div class="card' + (selected ? ' selected' : '') + '"';
    html += ' onclick="selPolicy(\'' + pol.id + '\')"';
...
function renderEvents() {
  var html = '';
  html += '<div class="screen anim" style="background:var(--bg);">';
  ...
  G.turnLog.forEach(function (log) {
    ...
    html += '자원 획득: ';
    var parts = [];
    Object.keys(log.gain).forEach(function (k) { parts.push((R_EMOJI[k] || '') + '+' + log.gain[k]); });
    html += parts.join(' ');
    html += '</div></div>';
  });
  html += '</div>';
  ...
  html += '<div style="text-align:center;">';
  html += '<button class="btn btn-primary" onclick="goSummary()">📊 이번 턴 결과 보기 →</button>';
  html += '</div>';
  html += '</div></div>';
  return html;
}
...
function renderSummary() {
  ...
  var html = '';
  html += '<div class="screen anim" style="background:var(--bg);justify-content:flex-start;padding-top:20px;">';
...
function renderResults() {
  ...
  catAwards.forEach(function (aw) {
    html += '<div style="background:rgba(0,0,0,0.3);border-radius:6px;padding:10px;">';
